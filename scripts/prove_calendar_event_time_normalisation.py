#!/usr/bin/env python3
"""Generate commit-bound TR-M24 calendar time-normalisation evidence."""
...
COMMANDS = (
    ...
    ("python3 -m unittest tests.test_calendar_event_time_normalisation", [sys.executable, "-m", "unittest", "tests.test_calendar_event_time_normalisation"]),
    ("python3 -m unittest tests.test_catalogue_api", [sys.executable, "-m", "unittest", "tests.test_catalogue_api"]),
)
REVIEWS = (("ai_engineer", "reviews/TR-M24_ai_engineer.md"), ("solution_architect", "reviews/TR-M24_solution_architect.md"), ("developer", "reviews/TR-M24_developer.md"))


def main() -> int:
    revision = subprocess.run(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True, capture_output=True, check=False).stdout.strip()
    if len(revision) != 40:
        raise RuntimeError("TR-M24 proof requires an inspectable Git commit")
    ...
    payload = {
        ...
        "environment": {"python": platform.python_version(), "platform": platform.platform(), "host": platform.node(), "runtime": "static template and synthetic-fixture validation"},
        ...
        "observed_result": "The catalogue exposes a time-normalisation template and synthetic fixtures that keep provider instants distinct from explicitly selected title metadata.",
    }
    path = ROOT / "runs" / "proofs" / "TR-M24_calendar_event_time_normalisation.json"
