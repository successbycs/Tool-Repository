...
COMMANDS = (
    ...
    (VERIFY_COMMAND, [sys.executable, "scripts/verify_cs_ai_lab_deploy.py", "--host", "Piwakawaka", "--account", "chris", "--release", "v0.1.0", "--source", str(ROOT), "--install-root", str(INSTALL_ROOT)]),
)
...
def main() -> int:
    ...
    payload = {
        ...
        "environment": {"python": platform.python_version(), "platform": platform.platform(), "host": platform.node(), "account": os.environ.get("USER", "")},
        ...
        "deployment": {"install_root": str(INSTALL_ROOT), "current_release": metadata},
        "observed_result": "A non-root Piwakawaka installation resolves v0.1.0 to an exact commit, validates the installed release, and retains a versioned rollback target.",
    }
