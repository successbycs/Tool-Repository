#!/usr/bin/env python3
"""Generate commit-bound TR-M25 public data-proxy control evidence."""
...
COMMANDS = (
    ...
    ("python3 -m unittest tests.test_public_readonly_data_proxy_pattern", [sys.executable, "-m", "unittest", "tests.test_public_readonly_data_proxy_pattern"]),
    ("python3 -m unittest tests.test_catalogue_api", [sys.executable, "-m", "unittest", "tests.test_catalogue_api"]),
)
REVIEWS = (("ai_engineer", "reviews/TR-M25_ai_engineer.md"), ("solution_architect", "reviews/TR-M25_solution_architect.md"), ("developer", "reviews/TR-M25_developer.md"))


def main() -> int:
    revision = subprocess.run(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True, capture_output=True, check=False).stdout.strip()
    if len(revision) != 40:
        raise RuntimeError("TR-M25 proof requires an inspectable Git commit")
    ...
    payload = {
        ...
        "environment": {"python": platform.python_version(), "platform": platform.platform(), "host": platform.node(), "runtime": "static template and control-pattern validation"},
        ...
        "observed_result": "The catalogue exposes a clean-room public-data proxy control template without publishing source proxy code, allowing arbitrary URLs, or centralising adapter execution.",
    }
    path = ROOT / "runs" / "proofs" / "TR-M25_public_readonly_data_proxy_controls.json"
